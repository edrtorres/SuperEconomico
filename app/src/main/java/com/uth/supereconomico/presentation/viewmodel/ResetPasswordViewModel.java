package com.uth.supereconomico.presentation.viewmodel;

import androidx.lifecycle.LiveData;
import androidx.lifecycle.MutableLiveData;
import androidx.lifecycle.ViewModel;
import com.uth.supereconomico.domain.repositories.AuthRepository;
import com.uth.supereconomico.domain.usecases.UpdatePasswordUseCase;

public class ResetPasswordViewModel extends ViewModel {
    private final UpdatePasswordUseCase updatePasswordUseCase;

    private final MutableLiveData<Boolean> _isSuccess = new MutableLiveData<>();
    public LiveData<Boolean> isSuccess = _isSuccess;

    private final MutableLiveData<String> _error = new MutableLiveData<>();
    public LiveData<String> error = _error;

    private final MutableLiveData<Boolean> _isLoading = new MutableLiveData<>();
    public LiveData<Boolean> isLoading = _isLoading;

    public ResetPasswordViewModel(UpdatePasswordUseCase updatePasswordUseCase) {
        this.updatePasswordUseCase = updatePasswordUseCase;
    }

    public void updatePassword(String newPassword, String accessToken) {
        _isLoading.setValue(true);
        updatePasswordUseCase.execute(newPassword, accessToken, new AuthRepository.Callback<Void>() {
            @Override
            public void onSuccess(Void result) {
                _isLoading.postValue(false);
                _isSuccess.postValue(true);
            }

            @Override
            public void onError(String message) {
                _isLoading.postValue(false);
                _error.postValue(message);
            }
        });
    }

    public void actualizarConTokenHash(String nuevaContrasena, String tokenHash) {
        _isLoading.setValue(true);
        updatePasswordUseCase.ejecutarConTokenHash(nuevaContrasena, tokenHash, new AuthRepository.Callback<Void>() {
            @Override
            public void onSuccess(Void result) {
                _isLoading.postValue(false);
                _isSuccess.postValue(true);
            }

            @Override
            public void onError(String message) {
                _isLoading.postValue(false);
                _error.postValue(message);
            }
        });
    }
}
